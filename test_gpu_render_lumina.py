"""GPU-Render-Test mit lumina_core (Default)."""
import numpy as np
import tempfile
import os
import subprocess
...
# Extrahiere mehrere Frames
for n in [0, 15, 29]:
    frame_png = tempfile.mktemp(suffix=f'_frame{n}.png')
    subprocess.run([
        'ffmpeg', '-y', '-i', output_mp4, '-vf', f'select=eq(n\,{n})', '-vframes', '1', frame_png
    ], capture_output=True)
    if os.path.exists(frame_png):
        from PIL import Image
        img = Image.open(frame_png).convert('RGB')
        arr = np.array(img)
        print(f"Frame {n}: shape={arr.shape}, min={arr.min()}, max={arr.max()}, mean={arr.mean():.1f}, std={arr.std():.1f}")
        os.unlink(frame_png)
